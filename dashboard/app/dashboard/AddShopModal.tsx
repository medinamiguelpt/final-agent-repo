...
function emptyBarber(): BarberEntry {
  return { _id: crypto.randomUUID(), name: "", services: [] };
}
...
const DEMO_POOL = [
  {
    ...
    email: "nikos@barberia.gr",
    address: "Ermou 42",
    city: "Athens",
    country: "Greece",
    website: "https://barberia-nikos.gr",
    description: "Classic Greek barbershop in the heart of Athens.",
  },
  {
    name: "The Greek Cut",
    phone: "+30 210 987 6543",
    email: "info@thegreekcut.com",
    address: "Mitropoleos 18",
    city: "Athens",
    country: "Greece",
    website: "https://thegreekcut.com",
    description: "Modern cuts and traditional hot-towel shaves.",
  },
  {
    name: "Acropolis Barbershop",
    phone: "+30 211 234 5678",
    email: "hello@acropolis.gr",
    address: "Adrianou 7",
    city: "Athens",
    country: "Greece",
    website: "https://acropolisbarber.gr",
    description: "Where ancient tradition meets modern style.",
  },
  {
    name: "Thessaloniki Blades",
    phone: "+30 2310 456 789",
    email: "blades@thessbarber.gr",
    address: "Tsimiski 55",
    city: "Thessaloniki",
    country: "Greece",
    website: "https://thessalonikiblades.gr",
    description: "Premium grooming in the second city of Greece.",
  },
  {
    name: "Kostas & Sons",
    phone: "+30 210 555 0011",
    email: "kostas@kostas-sons.gr",
    address: "Patission 101",
    city: "Athens",
    country: "Greece",
    website: "https://kostas-sons.gr",
    description: "Family barbershop since 1987.",
  },
  {
    name: "Crete Barbers Co.",
    phone: "+30 2810 303 030",
    email: "crete@barbersco.gr",
    address: "25is Avgoustou 12",
    city: "Heraklion",
    country: "Greece",
    website: "https://cretebarbers.gr",
    description: "Sun, sea and the perfect fade.",
  },
  {
    name: "Lisboa Cuts",
    phone: "+351 21 345 6789",
    email: "info@lisboacuts.pt",
    address: "Rua Augusta 100",
    city: "Lisbon",
    country: "Portugal",
    website: "https://lisboacuts.pt",
    description: "Premium grooming in Baixa-Chiado.",
  },
  {
    name: "Madrid Shave Club",
    phone: "+34 91 123 4567",
    email: "hola@madridshave.es",
    address: "Gran Via 28",
    city: "Madrid",
    country: "Spain",
    website: "https://madridshaveclub.es",
    description: "Old-school barbers, new-school attitude.",
  },
];
...
function pickDemo(): { form: FormState; hours: Hours } {
  ...
  return {
    ...
    hours: Object.fromEntries(DAYS.map(({ key }) => [key, { open: "09:00", close: "18:00", closed: key === "sun" }])),
  };
}
...
export default function AddShopModal({ open, onClose, onCreated, C }: Props) {
  ...
  // ── Mode selection ───────────────────────────────────────────────────────────
  function selectMode(m: CreationMode) {
    setMode(m);
    setStep("form");
  }
  function goBack() {
    ...
    setBarbers((prev) =>
      prev.map((b, i) => {
        ...
        return {
          ...b,
          services: has
            ? b.services.filter((s) => s.name !== svc.name)
            : [...b.services, { _id: crypto.randomUUID(), ...svc }],
        };
      }),
    );
  }
  ...
  function addCustomService(bi: number) {
    setBarbers((prev) =>
      prev.map((b, i) =>
        i === bi ? { ...b, services: [...b.services, { _id: crypto.randomUUID(), name: "", price: 0 }] } : b,
      ),
    );
  }
  ...
  function setServiceField(bi: number, si: number, patch: Partial<BarberService>) {
    setBarbers((prev) =>
      prev.map((b, i) =>
        i !== bi ? b : { ...b, services: b.services.map((s, j) => (j === si ? { ...s, ...patch } : s)) },
      ),
    );
  }
  ...
  function removeService(bi: number, si: number) {
    setBarbers((prev) =>
      prev.map((b, i) => (i !== bi ? b : { ...b, services: b.services.filter((_, j) => j !== si) })),
    );
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!migrationValid()) {
      setError("Please select at least one migration source, or check \u2018I\u2019ll add data manually\u2019");
      return;
    }
    setError("");
    setSubmitting(true);
    try {
      ...
      if (!session) {
        setError(sessErr?.message ?? "Session expired — please refresh the page");
        return;
      }
      ...
      const res = await fetch("/api/businesses", {
        ...
        body: JSON.stringify({
          ...
          extra_agent_id:
            mode === "migrate" && migration.elevenLabs && migration.elAgentId.trim()
              ? migration.elAgentId.trim()
              : null,
        }),
      });
      ...
      if (!res.ok) {
        setError(payload.error ?? "Business creation failed — please try again");
        return;
      }
      ...
      if (mode === "demo") {
        ...
        try {
          await generateDemoBookings(biz.id, biz.name, token, finalBarbers.length ? finalBarbers : undefined);
        } catch (err) {
          console.warn("[AddShopModal] demo data warn:", err);
        } finally {
          setGeneratingDemo(false);
        }
      }
      ...
      onCreated({
        ...
        barberData: finalBarberData.length ? finalBarberData : undefined,
      });
      onClose();
    } catch (err) {
  ...
  const chooseContent = (
    ...
    >
      ...
      >
        <div style={{ fontFamily: "var(--gbf-font-display)", fontSize: 18, fontWeight: 700, color: C.text }}>
          Add a barbershop
        </div>
        <button
          onClick={onClose}
      ...
      <div style={{ padding: "24px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {MODES.map(({ id, Icon, color, title, sub, badge }) => (
          ...
          >
            ...
            <ChevronDown
              size={14}
              style={{ color: C.textFaint, transform: "rotate(-90deg)", flexShrink: 0, marginTop: 4 }}
            />
          </button>
        ))}
      </div>
  ...
  const migrationSection = mode === "migrate" && (
    <div>
      ...
      >
        Import sources
        <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0, marginLeft: 6, color: C.textFaint }}>
          — select at least one
        </span>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <MigRow
          ...
          onToggle={() => {
            toggleMig("csvFile");
            if (!migration.csvFile) setTimeout(() => csvRef.current?.click(), 80);
          }}
        ...
        >
          {migration.csvFile && (
            <div style={{ marginTop: 8 }}>
              ...
              {migration.csvData && (
                <div
                  style={{ marginTop: 6, fontSize: 11, color: C.accent, display: "flex", alignItems: "center", gap: 5 }}
                >
  ...
  const formContent = (
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "var(--gbf-font-display)", fontSize: 16, fontWeight: 700, color: C.text }}>
            Add a barbershop
          </div>
        </div>
        {modeInfo && (
      ...
      <form onSubmit={handleSubmit} style={{ overflowY: "auto", flex: 1, display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 20, flex: 1 }}>
          ...
          <div>
            ...
            {filledFrom && (
              <div
                style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: C.accent }}
              >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            ...
            <div style={{ ...fw, gridColumn: "1/-1" }}>
              ...
              <textarea
                ...
                onChange={setField("description") as unknown as React.ChangeEventHandler<HTMLTextAreaElement>}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {DAYS.map(({ key, label }) => {
                ...
                return (
                  <div
                    key={key}
                    style={{ display: "grid", gridTemplateColumns: "44px 1fr", alignItems: "center", gap: 10 }}
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <button
                        ...
                        style={{
                          ...
                          position: "relative",
                          flexShrink: 0,
                          background: h.closed ? C.borderFaint : C.accent,
                      ...
                      {h.closed ? (
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <input
                            ...
                            style={{ ...inp, width: 100, padding: "5px 8px", fontSize: 13 }}
                          />
          ...
          <div>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                ...
                Team / Barbers
                <span
                  style={{ fontSize: 11, fontWeight: 500, textTransform: "none", letterSpacing: 0, color: C.textFaint }}
                >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {barbers.map((barber, bi) => (
                <div
                  key={barber._id}
                  style={{ border: `1.5px solid ${C.border}`, borderRadius: 14, overflow: "hidden" }}
                >
                  ...
                  >
                    ...
                    {barbers.length > MIN_BARBERS && (
                      <button
                        ...
                        onMouseLeave={(e) => {
                          (e.currentTarget as HTMLButtonElement).style.borderColor = C.borderFaint;
                  ...
                  <div style={{ padding: "12px 14px" }}>
                    ...
                    <div
                      style={{
                        ...
                        flexWrap: "wrap",
                        gap: 5,
                        marginBottom: barber.services.length > 0 ? 10 : 0,
                      }}
                    >
          ...
          >
            ...
            <div style={{ fontSize: 12, color: C.textMuted, lineHeight: 1.5 }}>
              The shared ElevenLabs agent will be automatically linked to this shop. All future calls made through it
              will be recorded under this shop&apos;s data.
            </div>
          </div>
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 10,
              border: "none",
              background: submitting || generatingDemo ? C.accentLight : C.accent,
              color: submitting || generatingDemo ? C.accentMid : C.bg,
              fontSize: 14,
              fontWeight: 800,
              cursor: submitting || generatingDemo ? "not-allowed" : "pointer",
              fontFamily: "inherit",
              display: "flex",
              alignItems: "center",
              gap: 8,
          ...
          >
            {(submitting || generatingDemo) && (
              <Loader size={14} style={{ animation: "gbf-spin 1s linear infinite" }} />
            )}
            {buttonLabel}
          </button>
        </div>
      ...
      >
        <div
          style={{
            ...
            height: 28,
            borderRadius: 7,
            background: checked ? `${C.accent}22` : C.borderFaint,
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            flexShrink: 0,

...
import { LayoutDashboard, ClipboardList, BarChart3, Scissors, Store, Palette, Monitor, Globe, User, ShieldCheck, Settings as SettingsIcon, Check, Phone, PhoneOff, Plus, LogOut, ChevronDown, PhoneCall, Inbox, Sun, Moon, CalendarDays, Coins, Languages, CornerUpLeft, Voicemail, Smartphone, X, AlertCircle, ChevronRight, Trophy, Sparkles, Trash2, Download, AlertTriangle, Loader, PersonStanding, ArrowRight, PanelTop, PanelBottom, Rows2, Rows3, Rows4 } from "lucide-react";
...
interface ServiceLine { service: string; barber: string; price: number; duration_minutes: number; }
interface AiBooking {
  conversation_id: string;
...
interface Settings {
  ...
  density: DensityKey; autoRefresh: boolean;
...
const DEFAULT_SETTINGS: Settings = {
  ...
  density: "comfortable", autoRefresh: true,
  navPosition: "top",
};
...
const DEMO_AI_BOOKINGS: AiBooking[] = [
  ...
  },
  {
    ...
    date: _pastDate(5), time: "11:30", price: 15, duration_secs: 101, message_count: 11,
...
function SkeletonCard() {
  return (
    <div style={{ background: "rgba(128,128,128,0.06)", border: "1px solid rgba(128,128,128,0.10)", borderRadius: "var(--gbf-radius-card)", padding: "20px 24px" }}>
...
const NAV_ITEMS: { id: SettingsSection; label: string; icon: React.ReactNode; desc: string }[] = [
  { id: "profile",    label: "Business Profile", icon: <Store         size={15} strokeWidth={1.75} />, desc: "Your barbershop info" },
  { id: "appearance", label: "Appearance",        icon: <Palette       size={15} strokeWidth={1.75} />, desc: "Theme & colours" },
  { id: "dashboard",  label: "Dashboard",         icon: <LayoutDashboard size={15} strokeWidth={1.75} />, desc: "Data & refresh" },
...
function SettingsPanel({
  ...
  langSettings, onLangUpdate, onAddShop, businesses, currentBiz, onSwitchBiz, onDeleteBiz,
}: {
  ...
  const section = (title: string, desc?: string) => (
    <div style={{ marginBottom: 18, marginTop: 28, paddingBottom: 10, borderBottom: `1px solid ${C.borderFaint}` }}>
      <div style={{ fontSize: 15, fontWeight: 700, color: C.text, fontFamily: "var(--gbf-font-display)", letterSpacing: "-0.02em" }}>{title}</div>
  ...
  const field = (label: string, value: string, onChange: (v: string) => void, opts?: { type?: string; placeholder?: string; readonly?: boolean; mono?: boolean; hint?: string }) => (
    <div style={{ marginBottom: 14 }}>
      ...
      <input
        ...
        style={{
          width: "100%", boxSizing: "border-box", padding: "10px 14px", borderRadius: "var(--gbf-radius-sm)",
          border: `1px solid ${C.border}`, background: opts?.readonly ? C.surfaceAlt : C.surface,
          color: C.text, fontSize: 14, fontFamily: opts?.mono ? "'JetBrains Mono', 'Fira Code', monospace" : "var(--gbf-font-sans)",
          outline: "none", transition: "border-color .2s, box-shadow .2s",
        }}
        onFocus={e => { if (!opts?.readonly) { e.target.style.borderColor = C.accent; e.target.style.boxShadow = `0 0 0 3px ${C.accentLight}`; } }}
  ...
  const pill = (label: string, active: boolean, onClick: () => void) => (
    <button onClick={onClick} className="gbf-btn" style={{
      flex: 1, minHeight: 40, padding: "8px 12px", borderRadius: "var(--gbf-radius-sm)",
      border: `1.5px solid ${active ? C.accent : C.border}`,
      background: active ? C.accentLight : C.surfaceAlt,
      color: active ? C.accent : C.textMuted,
      fontSize: 13, fontWeight: active ? 700 : 500, cursor: "pointer", fontFamily: "var(--gbf-font-sans)",
  ...
  const selectRow = (label: string, value: string, options: [string, string][], onChange: (v: string) => void) => (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: `1px solid ${C.borderFaint}` }}>
      ...
      <select value={value} onChange={e => onChange(e.target.value)} style={{
        ...
        borderRadius: "var(--gbf-radius-xs)", padding: "7px 12px", cursor: "pointer", outline: "none", fontFamily: "var(--gbf-font-sans)", minHeight: 38,
      }}>
  ...
  const ProfileSection = () => (
    <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
          {([["solo","Solo (just me)"],["small","2–5 barbers"],["medium","6–15 barbers"],["large","16+ barbers"]] as [BusinessProfile["size"], string][]).map(([val, lbl]) => (
            <button key={val} onClick={() => setProfileDraft(d => ({ ...d, size: val }))} className="gbf-btn" style={{
              padding: "10px 12px", borderRadius: "var(--gbf-radius-xs)", cursor: "pointer", fontFamily: "var(--gbf-font-sans)", fontSize: 13, fontWeight: 500,
              border: `1.5px solid ${profileDraft.size === val ? C.accent : C.border}`,
              background: profileDraft.size === val ? C.accentLight : C.surface,
              color: profileDraft.size === val ? C.accent : C.textMuted, transition: "all .2s", letterSpacing: "-0.01em",
            }}>{lbl}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <select value={profileDraft.country} onChange={e => setProfileDraft(d => ({ ...d, country: e.target.value }))} style={{ width: "100%", padding: "10px 14px", borderRadius: "var(--gbf-radius-sm)", border: `1px solid ${C.border}`, background: C.surface, color: C.text, fontSize: 14, fontFamily: "var(--gbf-font-sans)", outline: "none" }}>
      ...
      <button onClick={saveProfile} style={{
        marginTop: 20, width: "100%", padding: "13px", borderRadius: "var(--gbf-radius-sm)",
        background: profileSaved ? C.green : C.accent, color: "#fff",
        fontSize: 14, fontWeight: 700, border: "none", cursor: "pointer",
        fontFamily: "var(--gbf-font-display)", letterSpacing: "-0.01em", transition: "background .2s, transform .1s",
      }}>
  ...
  const AppearanceSection = () => (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 28, marginBottom: 6, paddingRight: 4 }}>
        <span style={{ fontSize: 10, color: C.textFaint, letterSpacing: "0.06em", display:"flex", alignItems:"center", gap:3 }}><Sun size={10} strokeWidth={2} /> LIGHT</span>
        <span style={{ fontSize: 10, color: C.textFaint, letterSpacing: "0.06em", display:"flex", alignItems:"center", gap:3 }}><Moon size={10} strokeWidth={2} /> DARK</span>
      </div>
        ...
        return (
          <div key={key} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 0", borderBottom: `1px solid ${C.borderFaint}` }}>
            ...
            <button onClick={() => onUpdate({ palette: key, mode: "light" })} title={`${p.name} Light`} className="gbf-btn" style={{ width: 40, height: 40, borderRadius: 10, cursor: "pointer", position: "relative", overflow: "hidden", background: p.light.bg, border: `2.5px solid ${(lightActive || sysActive) ? p.light.accent : "transparent"}`, boxShadow: (lightActive || sysActive) ? `0 0 0 1px ${p.light.accentMid}` : "0 1px 4px rgba(0,0,0,.12)", transition: "all .15s", flexShrink: 0 }}>
              ...
              {(lightActive || sysActive) && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}><Check size={13} strokeWidth={3} /></div>}
            </button>
            <button onClick={() => onUpdate({ palette: key, mode: "dark" })} title={`${p.name} Dark`} className="gbf-btn" style={{ width: 40, height: 40, borderRadius: 10, cursor: "pointer", position: "relative", overflow: "hidden", background: p.dark.bg, border: `2.5px solid ${darkActive ? p.dark.accent : "transparent"}`, boxShadow: darkActive ? `0 0 0 1px ${p.dark.accentMid}` : "0 1px 4px rgba(0,0,0,.3)", transition: "all .15s", flexShrink: 0 }}>
              ...
              {darkActive && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", color: p.dark.text }}><Check size={13} strokeWidth={3} /></div>}
            </button>
          </div>
        );
      })}
  ...
  const DashboardSection = () => (
    <div>
      ...
      {toggle("Auto-refresh", "Refreshes appointments and stats every 30 s. The live call feed always updates every 10 s regardless.", settings.autoRefresh, v => onUpdate({ autoRefresh: v }))}
      <div style={{ marginTop: 20, background: C.accentLight, borderRadius: "var(--gbf-radius-sm)", padding: "14px 16px", border: `1px solid ${C.border}` }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 4, fontFamily: "var(--gbf-font-display)", letterSpacing: "-0.01em" }}>Data sources</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {[["ElevenLabs Conversational AI","Connected"],["Appointment Ledger","Local data"],["Live call feed","Always · every 10 s"]].map(([name, note]) => (
            <div key={name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13 }}>
              ...
              <span style={{ color: C.green, fontWeight: 600, fontSize: 12, display: "flex", alignItems: "center", gap: 4 }}><Check size={12} strokeWidth={3} /> {note}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  const DisplaySection = () => (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {([
          ["compact",     <Rows4 size={20} />, "Compact",  "More rows visible, tighter padding"],
          ["comfortable", <Rows3 size={20} />, "Default",  "Balanced spacing for everyday use"],
          ["spacious",    <Rows2 size={20} />, "Spacious", "Larger cards, easier reading"],
        ] as [DensityKey, React.ReactNode, string, string][]).map(([val, icon, label, desc]) => {
          const active = settings.density === val;
          return (
            <button key={val} onClick={() => onUpdate({ density: val })} className="gbf-btn" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: "14px 10px", borderRadius: 14, cursor: "pointer", fontFamily: "var(--gbf-font-sans)", border: `2px solid ${active ? C.accent : C.border}`, background: active ? C.accentLight : C.surface, transition: "all .2s" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
        {([
          ["top",    <PanelTop size={20} />,    "Top",    "Classic — stays below the header"],
          ["bottom", <PanelBottom size={20} />, "Bottom", "App-style — pinned to the bottom for thumb reach"],
        ] as [Settings["navPosition"], React.ReactNode, string, string][]).map(([val, icon, label, desc]) => {
          const active = settings.navPosition === val;
          return (
            <button key={val} onClick={() => onUpdate({ navPosition: val })} className="gbf-btn" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 6, padding: "12px 14px", borderRadius: 14, cursor: "pointer", fontFamily: "var(--gbf-font-sans)", border: `2px solid ${active ? C.accent : C.border}`, background: active ? C.accentLight : C.surface, transition: "all .2s" }}>
              <span style={{ color: active ? C.accent : C.textMuted, display: "flex" }}>{icon}</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: active ? C.accent : C.text }}>{label}</span>
      ...
      <div style={{ fontSize: 11, color: C.textFaint, marginBottom: 16, display: "flex", alignItems: "center", gap: 5 }}>
        <Smartphone size={12} style={{ color: C.textFaint, flexShrink: 0 }} /><span>Bottom nav only activates on screens ≤ 768px wide</span>
      </div>
    </div>
  );
  ...
  // ── Section: Account ──────────────────────────────────────────────────────
  const PLANS = [
    { id: "demo",         name: "Demo",         price: "Free",     color: "#9AAABB", features: ["1 AI Agent","60 min/month","Basic analytics"] },
    { id: "starter",      name: "Starter",      price: "€29/mo",   color: "#3D7A50", features: ["1 AI Agent","500 min/month","Full analytics","Email support"] },
    { id: "professional", name: "Professional", price: "€79/mo",   color: "#1B5EBE", features: ["3 AI Agents","Unlimited min/month","Custom branding","Priority support"] },
  ...
  const AccountSection = () => (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px 0", borderBottom: `1px solid ${C.borderFaint}`, marginBottom: 4 }}>
        <div style={{ width: 52, height: 52, borderRadius: 16, background: `linear-gradient(135deg, ${C.accent}, ${C.accentMid})`, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, fontWeight: 800, flexShrink: 0, fontFamily: "var(--gbf-font-display)", letterSpacing: "-0.02em", boxShadow: `0 4px 12px ${C.accent}44` }}>{initials}</div>
        <div>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.text, fontFamily: "var(--gbf-font-display)", letterSpacing: "-0.02em" }}>{profile.businessName || "My Barbershop"}</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 4 }}>
        {(businesses && businesses.length > 0 ? businesses : [{ id: "", name: profile.businessName || "Greek Barber Festival", plan: undefined }]).map(shop => {
          ...
          return (
            <div key={shop.id || shop.name} style={{ borderRadius: 12, overflow: "hidden", border: `1px solid ${isDeleting ? C.red + "66" : isActive ? C.accent + "33" : C.border}`, background: isDeleting ? C.redLight : isActive ? C.accentLight : C.surface, transition: "all .15s" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px" }}>
                <div onClick={() => shop.id && onSwitchBiz?.(shop)} style={{ display: "flex", alignItems: "center", gap: 12, flex: 1, minWidth: 0, cursor: shop.id ? "pointer" : "default" }}>
                  <div style={{ width: 34, height: 34, borderRadius: 10, background: isDeleting ? C.red + "22" : isActive ? C.accent : C.surfaceAlt, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <Store size={16} color={isDeleting ? C.red : isActive ? "#fff" : C.textMuted} strokeWidth={2} />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: isDeleting ? C.red : C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{shop.name}</div>
                    <div style={{ fontSize: 11, color: C.textMuted }}>Owner · {isActive ? "Active" : "Tap to switch"}</div>
                  </div>
                </div>
                {shop.id && !isDeleting && (
                  <button onClick={() => setDeletingBiz({ id: shop.id, name: shop.name })}
                    className="gbf-btn"
                    title="Delete this barbershop"
                    style={{ width: 30, height: 30, borderRadius: 8, border: `1px solid ${C.border}`, background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "border-color .15s, background .15s" }}
                    onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = C.red; (e.currentTarget as HTMLButtonElement).style.background = C.redLight; }}
                ...
                {shop.id && isDeleting && (
                  <button onClick={() => setDeletingBiz(null)} className="gbf-btn"
                ...
                {isActive && !isDeleting && <Check size={14} style={{ color: C.accent, flexShrink: 0 }} strokeWidth={2.5} />}
              </div>

              {isDeleting && (
                <div style={{ padding: "0 14px 14px" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "10px 12px", background: C.redLight, border: `1px solid ${C.red}33`, borderRadius: 10, marginBottom: 10 }}>
                    <AlertTriangle size={14} style={{ color: C.red, flexShrink: 0, marginTop: 1 }} />
                    <div style={{ fontSize: 12, color: C.red, lineHeight: 1.5 }}>
                      <strong>This will permanently delete</strong> all data for <em>{shop.name}</em> — calls, appointments, agents, and the barbershop itself. This cannot be undone.
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <button
                      ...
                      onClick={async () => {
                        ...
                        const backup = { exported_at: new Date().toISOString(), business: bizData, calls: calls ?? [], appointments: appts ?? [] };
                      ...
                      style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 8, border: `1px solid ${C.border}`, background: C.surfaceAlt, color: C.text, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try { await onDeleteBiz?.(shop.id); setDeletingBiz(null); }
                        finally { setDeletingBiz(d => d ? { ...d, inProgress: false } : null); }
                      }}
                      style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 8, border: "none", background: C.red, color: "#fff", fontSize: 12, fontWeight: 700, cursor: inProgress ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: inProgress ? 0.7 : 1 }}>
                      {inProgress ? <Loader size={12} style={{ animation: "gbf-spin 1s linear infinite" }} /> : <Trash2 size={12} />}
                    ...
                    <button
                      ...
                      style={{ padding: "8px 14px", borderRadius: 8, border: `1px solid ${C.border}`, background: "transparent", color: C.textMuted, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
                      Cancel
                    </button>
                  </div>
                </div>
        ...
        <button onClick={() => onAddShop?.()} className="gbf-btn" style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 12, border: `1.5px dashed ${C.border}`, background: "transparent", cursor: "pointer", fontFamily: "inherit", width: "100%", transition: "border-color .15s, background .15s" }}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {PLANS.map(plan => {
          ...
          return (
            <div key={plan.id} style={{ border: `1.5px solid ${current ? plan.color : C.border}`, borderRadius: 16, padding: "14px 18px", background: current ? C.accentLight : C.surface, transition: "all .2s", boxShadow: current ? `0 2px 12px ${plan.color}18` : "none" }}>
              ...
              {!current && (
                <button className="gbf-btn" style={{ marginTop: 10, width: "100%", padding: "9px", borderRadius: "var(--gbf-radius-xs)", background: plan.color, color: "#fff", fontSize: 13, fontWeight: 700, border: "none", cursor: "pointer", fontFamily: "var(--gbf-font-display)", letterSpacing: "-0.01em" }}>
  ...
  const SecuritySection = () => (
    <div>
      ...
      {profile.twoFactorEnabled && (
        <div style={{ marginTop: 10, background: C.greenLight, borderRadius: "var(--gbf-radius-xs)", padding: "10px 14px", fontSize: 13, color: C.green, fontWeight: 600, display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
        {MOCK_SESSIONS.map(sess => (
          <div key={sess.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 14px", background: C.surfaceAlt, borderRadius: "var(--gbf-radius-sm)", border: `1px solid ${C.borderFaint}` }}>
            <div>
      ...
      <button className="gbf-btn" style={{ width: "100%", padding: "11px", borderRadius: "var(--gbf-radius-sm)", background: "transparent", border: `1px solid ${C.border}`, color: C.textMuted, fontSize: 13, cursor: "pointer", fontFamily: "var(--gbf-font-sans)", fontWeight: 500 }}>
        Sign out all other devices
      </button>

      {section("API & Integration")}
      <div style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: "var(--gbf-radius-sm)", padding: "12px 16px", marginBottom: 14 }}>
      ...
      {/* Sign out */}
      <button onClick={onLogout} className="gbf-btn" style={{ width: "100%", padding: "12px", borderRadius: "var(--gbf-radius-sm)", background: "transparent", border: `1px solid ${C.border}`, color: C.textMuted, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "var(--gbf-font-sans)", marginBottom: 10, letterSpacing: "-0.01em" }}>
        Sign out
      </button>
      ...
      <div style={{ border: `1px solid ${C.red}30`, borderRadius: 16, padding: "16px" }}>
        ...
        <input
          ...
          style={{ width: "100%", boxSizing: "border-box", padding: "10px 14px", borderRadius: "var(--gbf-radius-xs)", border: `1px solid ${C.border}`, background: C.surface, color: C.text, fontSize: 13, fontFamily: "var(--gbf-font-sans)", outline: "none", marginBottom: 8, transition: "border-color .2s" }}
        />
        <button disabled={deleteConfirm !== "DELETE"} className="gbf-btn" style={{
          width: "100%", padding: "11px", borderRadius: "var(--gbf-radius-xs)", fontSize: 13, fontWeight: 700, fontFamily: "var(--gbf-font-display)", letterSpacing: "-0.01em",
  ...
  const LanguageSection = () => (
    <div>
        ...
        {(Object.entries(LANG_META) as [LanguageKey, typeof LANG_META[LanguageKey]][]).map(([key, meta]) => {
          ...
          return (
            <button key={key} onClick={() => onLangUpdate({ lang: key })} className="gbf-btn" style={{
              ...
              borderRadius: "var(--gbf-radius-sm)", border: `1.5px solid ${active ? C.accent : C.border}`,
              background: active ? C.accentLight : C.surface, cursor: "pointer",
              fontFamily: "var(--gbf-font-sans)", textAlign: "left", transition: "all .2s", width: "100%",
            }}>
              <span style={{ fontSize: 22, flexShrink: 0 }}>{meta.flag}</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: active ? 700 : 500, color: active ? C.accent : C.text, letterSpacing: "-0.01em" }}>{meta.native}</div>
                <div style={{ fontSize: 12, color: C.textMuted }}>{meta.name}</div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                {meta.rtl && <span style={{ fontSize: 10, color: C.textFaint, background: C.surfaceAlt, padding: "2px 6px", borderRadius: 999, border: `1px solid ${C.borderFaint}` }}>RTL</span>}
      ...
      <div style={{ marginTop: 20, background: C.accentLight, borderRadius: "var(--gbf-radius-sm)", padding: "14px 16px", border: `1px solid ${C.border}` }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.textFaint, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>{t("previewLabel")}</div>
  ...
  return (
    <>
      ...
      <div className="gbf-settings-drawer" style={{ zIndex: 10001, background: C.surface, borderLeft: `1px solid ${C.border}`, boxShadow: "-2px 0 8px rgba(0,0,0,.04), -8px 0 32px rgba(0,0,0,.08), -24px 0 80px rgba(0,0,0,.12)", display: "flex", flexDirection: "column" }}>
        ...
        {/* Header */}
        <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10, position: "sticky", top: 0, background: `${C.surface}EE`, backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)", zIndex: 1, flexShrink: 0 }}>
          {/* Mobile back button */}
          ...
          <button onClick={onClose} className="gbf-btn gbf-icon-btn" style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: "50%", width: 44, height: 44, cursor: "pointer", color: C.textMuted, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "background .15s" }}>
        ...
        <div className="gbf-settings-body">
          {/* Nav sidebar */}
          <nav className={`gbf-settings-nav${mobContent ? " gbf-mob-hide" : ""}`} style={{ borderRight: `1px solid ${C.border}`, background: C.surfaceAlt, padding: "4px 0" }}>
            {NAV_ITEMS.map(item => {
              const active = activeSection === item.id;
              return (
                <button key={item.id} onClick={() => handleNavClick(item.id)} data-active={active ? "true" : undefined} className="gbf-btn gbf-settings-nav-btn" style={{
                  display: "flex", alignItems: "center", gap: 10, width: "calc(100% - 16px)",
                  margin: "2px 8px", padding: "9px 10px", borderRadius: 10,
                  background: active ? C.accentLight : "transparent",
                  border: "none",
                  cursor: "pointer", fontFamily: "inherit", textAlign: "left",
                  transition: "background .15s",
                }}>
                  <span style={{ width: 28, height: 28, borderRadius: 8, background: active ? C.accentMid + "22" : C.borderFaint, display: "flex", alignItems: "center", justifyContent: "center", color: active ? C.accent : C.textMuted, flexShrink: 0, transition: "all .2s" }}>{item.icon}</span>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: active ? 700 : 500, color: active ? C.accent : C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", letterSpacing: "-0.01em" }}>{t(`sNav${item.id.charAt(0).toUpperCase() + item.id.slice(1)}`)}</div>
                  </div>
                  {/* Mobile chevron */}
                  <svg className="gbf-mob-back" style={{ marginLeft: "auto", color: C.textFaint, flexShrink: 0 }} width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="5,2 11,7 5,12"/></svg>
                </button>
              );
            })}
          </nav>
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: "var(--gbf-radius-card)", padding: pad.card, position: "relative", overflow: "hidden",
    ...
    >
      ...
      {isClickable && (
        <div style={{ position: "absolute", top: 12, right: 14, color: C.accent, opacity: hov ? 1 : 0.28, transition: "opacity .18s, transform .18s", transform: hov ? "translateX(2px)" : "none", display: "flex" }}><ArrowRight size={13} strokeWidth={2} /></div>
      )}
      {/* Accent bar at top */}
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: hov && isClickable ? C.accent : C.accentLight, borderRadius: "20px 20px 0 0", transition: "background .22s" }} />
      <div key={String(value)} className="gbf-num-in gbf-stat-num" style={{ fontSize: 28, fontWeight: 800, color: C.accent, letterSpacing: "-0.6px", lineHeight: 1.05, fontFamily: "var(--gbf-font-display)" }}>{value}</div>
      <div className="gbf-stat-label" style={{ fontSize: 13, fontWeight: 600, color: C.text, marginTop: 5, letterSpacing: "-0.01em" }}>{label}</div>
      {sub && <div className="gbf-stat-sub" style={{ fontSize: 11, color: C.textMuted, marginTop: 2, fontWeight: 500 }}>{sub}</div>}
      {tip && hov && (
...
function Badge({ status, label, C }: { status: string; label: string; C: Colors }) {
  ...
  return <span style={{ background: c.bg, color: c.color, fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 99, textTransform: "uppercase", letterSpacing: "0.05em", whiteSpace: "nowrap" }}>{label}</span>;
}
...
function WalkInModal({ C, barbers, businessId, onClose, onSaved }: {
  C: Colors; barbers: string[]; businessId: string;
  ...
  const inp: React.CSSProperties = {
    ...
    background:C.surfaceAlt, color:C.text, fontSize:14, fontFamily:"inherit",
    outline:"none",
  };
  const sel: React.CSSProperties = { ...inp, cursor:"pointer" };
  const lbl: React.CSSProperties = { fontSize:11, fontWeight:700, color:C.textFaint, textTransform:"uppercase", letterSpacing:"0.07em", display:"block", marginBottom:5 };
  ...
  function fillDemo() {
    const firstNames = ["Alexandros","Dimitrios","Giorgos","Nikos","Kostas","Ioannis","Thanasis","Petros","Stelios","Makis","Manolis","Christos"];
    const lastNames  = ["Papadopoulos","Nikolaou","Georgiou","Ioannou","Alexiou","Petrou","Stavropoulos","Karamanlis","Lambrakis"];
    ...
    setPhone(`+30 69${String(Math.floor(Math.random()*90+10))} ${String(Math.floor(Math.random()*900+100))} ${String(Math.floor(Math.random()*900+100))}`);
  ...
  return createPortal(
    <div className="gbf-walkin-overlay" style={{ position:"fixed", inset:0, zIndex:1000, background:C.overlay }} onClick={e => { if(e.target===e.currentTarget) onClose(); }}>
      <div className="gbf-walkin-panel" style={{ background:C.surface, overflowY:"auto", padding:"28px 24px 32px", boxShadow:"0 -8px 40px rgba(0,0,0,.3)", animation:"gbf-slideU .25s cubic-bezier(.4,0,.2,1)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, gap:10 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
            <button type="button" onClick={fillDemo} className="gbf-btn" style={{ fontSize:11, fontWeight:700, color:C.accent, background:C.accentLight, border:`1px solid ${C.accent}44`, borderRadius:8, padding:"6px 12px", cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", gap:5, whiteSpace:"nowrap" }}>
              <Sparkles size={11} strokeWidth={2.5} /> Fill demo
            </button>
            <button type="button" onClick={onClose} style={{ width:44,height:44,borderRadius:10,border:`1px solid ${C.border}`,background:C.surfaceAlt,color:C.textMuted,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}><X size={18} /></button>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            <div>
              ...
              <input value={name} onChange={e=>setName(e.target.value)} placeholder="Name" required style={inp} autoFocus />
            </div>
            <div>
              <label style={lbl}>Phone</label>
              <input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="+30 694..." style={inp} type="tel" />
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {services.map((svc, i) => (
                <div key={i} style={{ display:"grid", gridTemplateColumns:"1fr 1fr 80px 72px 32px", gap:6, alignItems:"center" }}>
                  <select value={svc.service} onChange={e=>updateSvc(i,{service:e.target.value})} style={{ ...sel, padding:"8px 10px", fontSize:13 }}>
                    {SERVICE_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                  <select value={svc.barber} onChange={e=>updateSvc(i,{barber:e.target.value})} style={{ ...sel, padding:"8px 10px", fontSize:13 }}>
                    <option value="">— Any —</option>
                    {barbers.map(b => <option key={b} value={b}>{b}</option>)}
                  </select>
                  <select value={svc.duration_minutes} onChange={e=>updateSvc(i,{duration_minutes:parseInt(e.target.value)})} style={{ ...sel, padding:"8px 6px", fontSize:13 }}>
                    {[15,20,30,45,60,90,120].map(d => <option key={d} value={d}>{d}m</option>)}
                  </select>
                  <input
                    value={svc.price}
                  ...
                  <button
                    ...
                    style={{ width:32, height:32, borderRadius:8, border:`1px solid ${C.border}`, background:C.surfaceAlt, color:services.length === 1 ? C.textFaint : C.red, cursor:services.length === 1 ? "default" : "pointer", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, fontSize:16, opacity:services.length === 1 ? 0.3 : 1 }}
                  >×</button>
                </div>
              ))}
            </div>
            ...
            <button
              type="button"
              onClick={() => setServices(prev => [...prev, { service: SERVICE_OPTIONS[0], barber: barbers[0] ?? "", price: "", duration_minutes: 30 }])}
              style={{ marginTop:8, padding:"7px 14px", borderRadius:8, border:`1.5px dashed ${C.accent}66`, background:"transparent", color:C.accentMid, fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", gap:5 }}
            >+ Add service</button>
            {/* Total summary */}
            {services.length > 0 && (totalPrice > 0 || totalDuration > 0) && (
              <div style={{ marginTop:6, fontSize:12, color:C.textMuted }}>
                Total: <span style={{ fontWeight:700, color:C.accent }}>€{totalPrice.toFixed(2)}</span> · <span style={{ fontWeight:700, color:C.accent }}>{totalDuration} min</span>
              </div>
            )}
          </div>
          ...
          {err && <div style={{ fontSize:13, color:C.red, background:C.redLight, padding:"10px 14px", borderRadius:8 }}>{err}</div>}

          <button type="submit" disabled={saving} style={{ padding:"14px", borderRadius:12, border:"none", background:saving ? C.accentLight : C.accent, color:saving ? C.accentMid : C.bg, fontSize:15, fontWeight:800, cursor:saving?"not-allowed":"pointer", fontFamily:"inherit", marginTop:4, transition:"background .15s" }}>
...
function HubTab({ agent, C, density, loading, profile, conversations, aiBookings, liveCall, onTabChange, onConvSelect, callActive, callStatus, liveTranscript, agentMode, onHumanTakeover, businesses, currentBiz }: {
  agent: AgentData | null; C: Colors; density: DensityKey;
  loading: boolean; profile: BusinessProfile;
  ...
  const aiTodayBookings = aiBookings.filter(b => b.date === d0 && b.call_status !== "error" && b.status !== "failed" && b.status !== "cancelled");
  ...
  const totalTodayCount = usingRealData ? aiTodayBookings.length : (aiTodayBookings.length > 0 ? aiTodayBookings.length : mockTodayAppts.length);
  const aiTodayRevenue  = aiTodayBookings.reduce((s, b) => s + (b.price || 0), 0);
  const revenue         = usingRealData
    ? aiTodayRevenue
    : (aiTodayRevenue > 0 ? aiTodayRevenue : mockTodayAppts.reduce((s, a) => s + a.price, 0));
  ...
  const allFeedEntries: FeedEntry[] = aiBookings.map(b => {
    ...
    if (b.call_status === "in-progress" || b.call_status === "processing") {
      ...
      feedType = "NO ANSWER";   feedColor = C.textFaint; feedBg = C.borderFaint;
      ...
      feedType = "PENDING";     feedColor = C.amber;     feedBg = C.amberLight;
  ...
  const feedEntries = hideNoAnswer
    ? allFeedEntries.filter(e => e.feedType !== "NO ANSWER")
    : allFeedEntries;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: pad.gap }}>
      ...
      {isAllShops && businesses && (
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 16px", borderRadius:12, background:C.accentLight, border:`1px solid ${C.accent}33` }}>
          <Store size={14} style={{ color:C.accent, flexShrink:0 }} />
          <span style={{ fontSize:13, fontWeight:600, color:C.accent }}>All shops</span>
          <span style={{ fontSize:12, color:C.textMuted }}>— showing aggregated data from {businesses.length} barbershops:</span>
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            {businesses.map(b => (
              <span key={b.id} style={{ fontSize:11, fontWeight:600, padding:"2px 8px", borderRadius:99, background:C.surface, border:`1px solid ${C.border}`, color:C.text }}>{b.name}</span>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="gbf-kpi-strip gbf-stagger" style={{ marginBottom: 8 }}>
        {[
          { icon: <span style={{ width:8, height:8, borderRadius:"50%", background: liveCall ? C.red : C.green, display:"inline-block" }} />, val: liveCall ? "LIVE" : "READY", label: "Status", color: liveCall ? C.red : C.green, bg: liveCall ? C.redLight : C.greenLight },
          { icon: <PhoneCall size={13} strokeWidth={2} />, val: callCount, label: t("callsWeek"), color: C.accent, bg: C.accentLight },
          { icon: <CalendarDays size={13} strokeWidth={2} />, val: totalTodayCount, label: t("bookingsToday"), color: C.green, bg: C.greenLight },
          { icon: <Coins size={13} strokeWidth={2} />, val: `€${revenue}`, label: t("revenueToday"), color: C.amber, bg: C.amberLight },
          { icon: <Languages size={13} strokeWidth={2} />, val: languages.length, label: t("languages"), color: C.accent, bg: C.accentLight },
        ].map(({ icon, val, label, color, bg }) => (
          <div key={label} className="gbf-chip" style={{ background: bg, border: `1px solid ${color}33`, color: C.text, gap: 5 }}>
      ...
      <div className="gbf-stat-grid gbf-stagger">
        ...
        <StatCard value={`€${revenue}`}    label={t("revenueToday")}  sub={usingRealData ? t("fromAICalls") : t("demoData")} C={C} density={density} loading={loading} onClick={() => onTabChange("analytics")} tip="View Analytics" />
      ...
      <div style={{ gap: pad.gap }} className="gbf-hub-main">
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 20, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          {callActive ? (
            <>
              {/* Mode indicator bar */}
              <div style={{ padding: "10px 20px", borderBottom: `1px solid ${C.borderFaint}`, display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{
                  width: 8, height: 8, borderRadius: "50%", display: "inline-block", flexShrink: 0,
                  background: agentMode === "speaking" ? C.accent : agentMode === "listening" ? C.green : agentMode === "thinking" ? C.amber : C.textFaint,
                  animation: agentMode !== "idle" ? "gbf-pulse 0.8s infinite" : "none",
                }} />
                <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", color: agentMode === "speaking" ? C.accent : agentMode === "listening" ? C.green : agentMode === "thinking" ? C.amber : C.textFaint }}>
                  {agentMode === "speaking" ? "Agent speaking" : agentMode === "listening" ? "Listening…" : agentMode === "thinking" ? "Processing…" : "Connected"}
                </span>
                <span style={{ marginLeft: "auto", fontSize: 10, color: C.textFaint, fontVariantNumeric: "tabular-nums" }}>
                  {liveTranscript.length} messages
                </span>
              </div>

              {/* Transcript scroll area */}
              <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px", display: "flex", flexDirection: "column", gap: 10, minHeight: 220, maxHeight: 340 }}>
                {liveTranscript.length === 0 ? (
                  <div style={{ textAlign: "center", padding: "32px 0", color: C.textFaint, fontSize: 12 }}>
                    Waiting for conversation to begin…
                  </div>
                ) : liveTranscript.map((msg) => (
                  <div key={msg.ts} className="gbf-msg-bubble" style={{ display: "flex", flexDirection: "column", alignItems: msg.role === "agent" ? "flex-start" : "flex-end" }}>
                    <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", color: C.textFaint, marginBottom: 3, paddingLeft: msg.role === "agent" ? 4 : 0, paddingRight: msg.role === "user" ? 4 : 0 }}>
              ...
              <div style={{ padding: "12px 16px", borderTop: `1px solid ${C.borderFaint}` }}>
                <button
                  ...
                  style={{
                    ...
                    cursor: "pointer", fontFamily: "var(--gbf-font-sans)", letterSpacing: "-0.01em",
              ...
              <div style={{ padding: "28px 20px 20px", display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
                ...
                <div style={{ position: "relative" }}>
                  <div className={liveCall ? "" : "gbf-ring-idle"} style={{ width: 120, height: 120, borderRadius: "50%", border: `2px solid ${C.borderFaint}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ width: 96, height: 96, borderRadius: "50%", border: `2px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <div style={{ width: 72, height: 72, borderRadius: "50%", background: liveCall ? C.redLight : C.accentLight, border: `2px solid ${liveCall ? C.red + "44" : C.accentMid + "44"}`, display: "flex", alignItems: "center", justifyContent: "center", animation: liveCall ? "gbf-pulse 2s infinite" : "none", transition: "background .3s, border-color .3s" }}>
                ...
                <div style={{ textAlign: "center" }}>
                  {loading
                    ...
                    : <div style={{ fontFamily: "var(--gbf-font-display)", fontSize: 20, fontWeight: 600, color: C.text }}>{agentName}</div>
                  }
                ...
                <div style={{ width: "100%", background: callStatus === "connecting" ? C.amberLight : C.accentLight, border: `1px dashed ${callStatus === "connecting" ? C.amber : C.accentMid}`, borderRadius: 12, padding: "12px 16px", textAlign: "center", transition: "background .3s" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: callStatus === "connecting" ? C.amber : C.accent }}>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 20, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.borderFaint}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              {liveCall && (
                <div style={{ display: "flex", alignItems: "center", gap: 5, background: C.redLight, padding: "4px 10px", borderRadius: 99 }}>
              ...
              {noAnswerCount > 0 && (
                <button
                  ...
                  style={{
                    ...
                    padding: "4px 9px", borderRadius: 6, lineHeight: 1.4,
          ...
          {/* Feed entries */}
          <div className="gbf-stagger gbf-feed-scroll" style={{ overflowY: "auto", minHeight: 200, maxHeight: "min(640px, 55vh)", padding: "16px 16px", display: "flex", flexDirection: "column", gap: 16 }}>
            {feedEntries.length === 0 ? (
              <div style={{ padding: "48px 24px", textAlign: "center" }}>
                <div style={{ display:"flex", justifyContent:"center", marginBottom:12 }}>
                  <div style={{ width:56, height:56, borderRadius:16, background:C.accentLight, display:"flex", alignItems:"center", justifyContent:"center" }}>
                ...
                <div style={{ fontSize: 13, color: C.textMuted, lineHeight: 1.5 }}>
                  Use the call button (bottom-right) to make a call.<br />It will appear here in real time.
                </div>
              </div>
              ...
              const rawSummary = entry.summary
                ? entry.summary.replace(/^(The client|The caller|Client|Caller)\b[^.]*?\bcalled\b[^.]*?[.]/i, "").replace(/^[\s,\.]+/, "").trim()
                : "";
              ...
              if (entry.services && entry.services.length > 1) {
                ...
                uniqueBarbers.forEach((b: string) => { if (b && b !== "TBD") tags.push(b); });
              } else {
              ...
              return (
                <div key={entry.conversation_id}
                  ...
                  onClick={() => { navigator.vibrate?.(8); onConvSelect(entry.conversation_id); }}
                  ...
                  <div style={{ padding: "11px 14px 10px" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                      <span style={{
                        ...
                        color: entry.feedColor, background: entry.feedBg,
                        ...
                        {isLiveEntry && (
                          <span style={{ width: 5, height: 5, borderRadius: "50%", background: entry.feedColor, display: "inline-block", animation: "gbf-pulse 0.8s infinite" }} />
                        )}
                        {entry.feedType}
                      </span>
                      <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                        {entry.call_language && entry.call_language !== "el" && (
                          <span style={{ fontSize: 10, fontWeight: 700, color: C.accentMid, background: C.accentLight, border: `1px solid ${C.accent}22`, padding: "2px 6px", borderRadius: 4, letterSpacing: "0.06em" }}>
                    ...
                    <div style={{ fontSize: 15, fontWeight: 800, color: C.text, lineHeight: 1.2, letterSpacing: "-0.01em", wordBreak: "break-word" }}>
                      {entry.client_name}
                    </div>
                  </div>
                  ...
                  {(tags.length > 0 || entry.duration_secs > 0) && (
                    <div style={{
                      ...
                      {entry.duration_secs > 0 && (
                        <span style={{ fontSize: 10, color: C.textFaint, marginLeft: "auto", fontVariantNumeric: "tabular-nums" }}>
          ...
          {/* Feed footer */}
          {feedEntries.length > 0 && (
            <div style={{ borderTop: `1px solid ${C.borderFaint}`, padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: C.accentLight, border: `1px solid ${C.accentMid ?? C.accent}33`,
                ...
                View full history in Ledger <ArrowRight size={12} strokeWidth={2} style={{ display:"inline", verticalAlign:"middle", marginLeft:2 }} />
              </button>
            </div>
          )}
        </div>
...
// ── LedgerTab ─────────────────────────────────────────────────────────────────
function LedgerTab({ C, density, aiBookings, liveCall, selectedConvId, onConvSelected, businesses, isAllShops, currentBiz, onRefreshData }: {
  ...
  async function regenerateDemoData() {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const token = session?.access_token;
      if (!token) { setRegenMsg("Not signed in."); return; }
      ...
      const data = await res.json();
      if (!res.ok) { setRegenMsg(data.error ?? "Failed."); return; }
  ...
  function scrollAndCenter(id: string, highlight = false) {
    ...
    function attempt() {
      ...
      if (!trigger) { if (n++ < 20) requestAnimationFrame(attempt); return; }
      ...
      if (!detailEl) { if (n++ < 20) requestAnimationFrame(attempt); return; }
  ...
  const [cancelled, setCancelled] = useState<Record<string, CancelEntry>>(() => {
    try { return JSON.parse(localStorage.getItem("gbf-cancelled") ?? "{}"); } catch { return {}; }
  });
  ...
  function confirmCancel() {
    ...
    setCancelled(next);
    try { localStorage.setItem("gbf-cancelled", JSON.stringify(next)); } catch {}
    setCancelModal(null);
  }
  ...
  const fetchTranscript = useCallback(async (id: string, summary?: string) => {
    ...
    if (id.startsWith("demo_")) {
      setTranscripts(prev => {
        const next = { ...prev, [id]: { conversation_id: id, status: "done", transcript: DEMO_TRANSCRIPTS[id] ?? [], metadata: { start_time_unix_secs: 0, call_duration_secs: 0 }, analysis: { transcript_summary: summary ?? "" } } };
  ...
  const demoEntries: UnifiedEntry[] = APPOINTMENTS.map(a => ({ ...a, id: String(a.id), source: "manual" as SourceKey }));
  const aiEntries: UnifiedEntry[]   = aiBookings.map(b => ({
    ...
    source:       (b.source === "walk-in" ? "walk-in"
                  : b.source === "website" ? "website"
  ...
  const allEntriesWithLocal = allEntries.map(a =>
    cancelled[a.id] ? { ...a, status: "cancelled" } : a
  );
  ...
  const filtered = allEntriesWithLocal.filter(a =>
  ...
  const todayActive = allEntriesWithLocal.filter(a =>
    a.date === d0 && a.status !== "cancelled" && a.status !== "failed" && a.call_status !== "error"
  );

  // Derive barbers from actual entries (deduplicated)
  const availableBarbers = Array.from(new Set(
    allEntriesWithLocal.flatMap(a => a.services?.map(s => s.barber).filter(Boolean) ?? (a.barber && a.barber !== "TBD" ? [a.barber] : []))
  )).sort();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: pad.gap }}>
      ...
      {cancelModal && typeof document !== "undefined" && createPortal(
        <>
          {/* Backdrop */}
          <div onClick={() => setCancelModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.55)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", zIndex: 2000 }} />
          {/* Dialog — centered on desktop, bottom sheet on mobile via .gbf-cancel-modal */}
          <div className="gbf-cancel-modal" style={{ background: C.surface, border: `1px solid ${C.border}`, boxShadow: "0 24px 80px rgba(0,0,0,.45)", display: "flex", flexDirection: "column" }}>
            {/* Mobile drag handle */}
            <div style={{ display: "flex", justifyContent: "center", padding: "10px 0 2px" }}>
              <div className="gbf-settings-handle" style={{ width: 36, height: 4, borderRadius: 99, background: C.border }} />
            </div>
            {/* Header — sticky so title stays visible when body scrolls */}
            <div style={{ padding: "16px 20px 14px", borderBottom: `1px solid ${C.border}`, background: C.surfaceAlt, flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: C.textMuted, marginTop: 4, wordBreak: "break-word" }}>{cancelModal.label}</div>
                </div>
                <button onClick={() => setCancelModal(null)} className="gbf-btn gbf-icon-btn" style={{ background: C.borderFaint, border: "none", borderRadius: 99, width: 44, height: 44, minWidth: 44, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: C.textMuted, flexShrink: 0 }}><X size={16} /></button>
              </div>
            </div>
            {/* Body — scrollable if content overflows on small screens */}
            <div style={{ padding: "20px 20px 8px", overflowY: "auto", flex: 1 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: C.textFaint, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>{t("cancelReason")}</label>
              <select
                value={cancelReason}
                onChange={e => setCancelReason(e.target.value)}
                style={{ width: "100%", fontSize: 14, color: C.text, background: C.surfaceAlt, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: "11px 12px", fontFamily: "inherit", outline: "none", cursor: "pointer", marginBottom: 16, minHeight: 44 }}
              >
                {CANCEL_REASONS.map(r => <option key={r} value={r}>{r}</option>)}
              </select>
              <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: C.textFaint, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>
              ...
              <textarea
                ...
                style={{ width: "100%", fontSize: 14, color: C.text, background: C.surfaceAlt, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: "11px 12px", fontFamily: "inherit", outline: "none", resize: "vertical", boxSizing: "border-box" }}
              />
            </div>
            {/* Footer — sticky at bottom, full-width buttons on mobile */}
            <div style={{ padding: "12px 20px 16px", borderTop: `1px solid ${C.border}`, display: "flex", gap: 10, flexShrink: 0 }}>
              <button onClick={() => setCancelModal(null)} className="gbf-btn" style={{ flex: 1, padding: "12px 16px", borderRadius: 12, border: `1.5px solid ${C.border}`, background: "transparent", color: C.textMuted, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", minHeight: 48 }}>
                {t("keepAppt")}
              </button>
              <button onClick={confirmCancel} className="gbf-btn" style={{ flex: 2, padding: "12px 16px", borderRadius: 12, border: "none", background: C.red, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", minHeight: 48 }}>
      ...
      <div className="gbf-stat-grid gbf-stagger">
        ...
        <StatCard value={active.length}  label={t("appointments")}  sub={`${active.filter(a => a.isAiCall).length} AI · ${active.filter(a => a.source === "walk-in").length} walk-in · ${active.filter(a => a.source === "manual").length} manual`} C={C} density={density} />
      </div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px 16px" }}>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8, gap: 8 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: C.textFaint, textTransform: "uppercase", letterSpacing: "0.08em" }}>{t("srcSource")}</span>
            <button
              onClick={() => setFilterSources(filterSources.size === ALL_SOURCES.length ? new Set() : new Set(ALL_SOURCES))}
              className="gbf-btn"
              style={{ fontSize: 11, fontWeight: 700, color: C.textMuted, background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: 6, padding: "3px 10px", cursor: "pointer", fontFamily: "inherit" }}
            >
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    checked ? next.delete(src) : next.add(src);
                ...
                >
                  <span style={{ display: "flex", alignItems: "center", width: 14, height: 14, border: `1.5px solid ${checked ? cfg.color : C.border}`, borderRadius: 4, background: checked ? cfg.color : "transparent", justifyContent: "center", flexShrink: 0, transition: "all .15s" }}>
        ...
        {(() => {
          const anyActive = filterDate !== "all" || filterBarber !== "all" || filterStatus !== "all" || filterShop !== "all" || filterSources.size !== ALL_SOURCES.length;
          ...
          const filterBtn = (
            ...
            return (
              <div key={id} style={{ position: "relative" }}>
                ...
                >
                  <span>{display}</span>
                  <ChevronDown size={12} style={{ transition: "transform .15s", transform: isOpen ? "rotate(180deg)" : "none", flexShrink: 0 }} />
                </button>
                {isOpen && (
                  <>
                    {createPortal(
                    ...
                    <div style={{
                      position: "absolute", top: "calc(100% + 6px)", left: 0, zIndex: 151,
                      ...
                      {options.map(([v, l]) => {
                        ...
                        return (
                          <button
                            key={v}
                            onClick={() => { onSelect(v); setOpenFilter(null); }}
                            className="gbf-btn"
                            style={{
          ...
          const dateOptions: [string,string][] = [["all", t("allDates")], ...dates.map(d => [d, d] as [string,string])];
          const barberOptions: [string,string][] = [["all", t("allBarbers")], ...availableBarbers.map(b => [b, b] as [string,string])];
          const statusOptions: [string,string][] = [["all", t("allStatuses")],["confirmed", t("confirmed")],["in-progress", t("inProgress")],["pending", t("pending")],["cancelled", t("cancelled")]];
          const shopOptions: [string,string][] = [["all", "All shops"], ...(businesses ?? []).map(b => [b.id, b.name] as [string,string])];
          ...
          return (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center" }}>
              ...
              {isAllShops && businesses && businesses.length > 1 &&
                filterBtn("shop", "All shops", filterShop, shopOptions, setFilterShop)}
              {anyActive && (
                <button
                  onClick={() => { setFilterDate("all"); setFilterBarber("all"); setFilterStatus("all"); setFilterShop("all"); setFilterSources(new Set(ALL_SOURCES)); }}
                  className="gbf-btn"
                  style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 12px", borderRadius: 10, border: `1px solid ${C.border}`, background: "transparent", color: C.textMuted, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", transition: "all .15s" }}
                >
        ...
        <div style={{ marginTop: 10, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <span style={{ fontSize: 12, color: C.textMuted }}>{filtered.length} {t("apptsShown")} {aiBookings.length > 0 && `· ${aiBookings.length} ${t("realAICalls")}`}</span>
            {noAnswerCount > 0 && (
              <button
                onClick={() => setHideNoAnswer(h => !h)}
                title={hideNoAnswer ? "Show no-answer calls" : "Hide no-answer calls"}
                style={{
                  ...
                  padding: "4px 10px", borderRadius: 6, lineHeight: 1.4,
                  background: hideNoAnswer ? C.accentLight : C.borderFaint,
                  color: hideNoAnswer ? C.accent : C.textFaint,
                  transition: "background .15s, color .15s",
                }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            {todayActive.length > 0 && (
              <button
                onClick={() => openCancelModal({ ids: todayActive.map(a => a.id), mode: "all-today", label: `${todayActive.length} appointment${todayActive.length !== 1 ? "s" : ""} on ${d0}` })}
                className="gbf-btn"
                style={{ fontSize: 11, fontWeight: 700, color: C.red, background: C.redLight, border: `1px solid ${C.red}44`, borderRadius: 8, padding: "5px 12px", cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 4 }}
              >
            ...
            {currentBiz && (currentBiz.plan === "demo" || currentBiz.plan === "free") && (
              <button
                ...
                className="gbf-btn"
                style={{ fontSize: 11, fontWeight: 700, color: C.textMuted, background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: 8, padding: "5px 12px", cursor: regenLoading ? "default" : "pointer", fontFamily: "inherit", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 4, opacity: regenLoading ? 0.6 : 1, transition: "all .15s" }}
              >
                {regenLoading ? <Loader size={11} style={{ animation: "spin 1s linear infinite" }} /> : <CornerUpLeft size={11} />}
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8 }}>
            {expandedCalls.size > 0 && (
              <button
                ...
                className="gbf-btn"
                style={{ fontSize: 12, fontWeight: 700, color: C.textMuted, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5, transition: "all .15s" }}
              >
            ...
            {!allExpanded && (
              <button
                ...
                className="gbf-btn"
                style={{ fontSize: 12, fontWeight: 700, color: C.accent, background: C.accentLight, border: `1px solid ${C.accent}44`, borderRadius: 8, padding: "6px 14px", cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5, transition: "all .15s" }}
              >
      ...
      <div className="gbf-ledger-table" style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {filtered.length === 0
                ? <tr><td colSpan={9} style={{ padding: 40, textAlign: "center", color: C.textMuted, fontSize: 14 }}>
                    ...
                    <>
                      <tr key={a.id} id={`dt-ledger-entry-${a.id}`} className={`gbf-tr-hover${isAI ? " gbf-ai-row" : ""}`} style={{ background: isLiveRow ? C.amberLight : (i % 2 === 0 ? C.surface : C.surfaceAlt), cursor: isAI ? "pointer" : "default" }}
                        ...
                        <td style={{ padding: pad.row, borderBottom: `1px solid ${C.borderFaint}` }}>
                          ...
                          {isAllShops && isAI && a.business_name && (
                            <div style={{ fontSize: 10, fontWeight: 600, color: C.accent, marginTop: 2, display:"flex", alignItems:"center", gap:3 }}>
                              <Store size={9} /> {a.business_name}
                            </div>
                          )}
                        </td>
                        <td style={{ padding: pad.row, fontSize: 13, color: C.textMuted, borderBottom: `1px solid ${C.borderFaint}`, whiteSpace: "nowrap" }}>
                        ...
                        <td style={{ padding: pad.row, borderBottom: `1px solid ${C.borderFaint}` }}>
                          {a.services && a.services.length > 1
                            ? (() => {
                                const uniqueBarbers = [...new Set(a.services.map((s: ServiceLine) => s.barber).filter(Boolean))];
                                return <span style={{ background: C.accentLight, color: C.accent, fontSize: 12, fontWeight: 600, padding: "3px 10px", borderRadius: 99 }}>{uniqueBarbers.join(" · ") || a.barber}</span>;
                              })()
                        ...
                        <td style={{ padding: pad.row, borderBottom: `1px solid ${C.borderFaint}`, width: 32 }}>
                          {(a.isAiCall || a.source === "website") && (
                            <span style={{ fontSize: 9, fontWeight: 800, color: "#6C63FF", background: "#6C63FF18", border: "1px solid #6C63FF44", borderRadius: 4, padding: "2px 5px", letterSpacing: "0.04em" }}>AI</span>
                          )}
                        </td>
                        <td style={{ padding: pad.row, borderBottom: `1px solid ${C.borderFaint}` }}>
                          <div style={{ display: "flex", gap: 5, alignItems: "center", flexWrap: "wrap" }}>
                            <Badge status={a.status} label={tStatus(a.status)} C={C} />
                            {cancelled[a.id] && <span title={`${cancelled[a.id].reason}${cancelled[a.id].note ? ` — ${cancelled[a.id].note}` : ""}`} style={{ fontSize: 10, color: C.textFaint, cursor: "help" }}>ℹ</span>}
                            {a.status !== "cancelled" && a.call_status !== "error" && (
                              <button
                                onClick={e => { e.stopPropagation(); openCancelModal({ ids: [a.id], mode: "single", label: `${a.name} · ${a.service} · ${a.date} ${a.time}` }); }}
                                className="gbf-btn"
                                style={{ fontSize: 10, fontWeight: 700, color: C.red, background: "transparent", border: `1px solid ${C.red}55`, borderRadius: 6, padding: "5px 10px", minHeight: 28, cursor: "pointer", fontFamily: "inherit", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}
                              ><X size={11} /></button>
                            )}
                          </div>
                        </td>
                      ...
                      {isOpen && isAI && (
                        <tr key={`${a.id}-detail`} id={`dt-ledger-detail-${a.id}`}>
                          <td colSpan={9} style={{ background: C.surfaceAlt, borderBottom: `1px solid ${C.border}`, padding: "12px 20px" }}>
      ...
      <div className="gbf-ledger-cards">
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "48px 24px", color: C.textMuted }}>
            <div style={{ display:"flex", justifyContent:"center", marginBottom:10 }}><div style={{ width:52, height:52, borderRadius:14, background:C.surfaceAlt, display:"flex", alignItems:"center", justifyContent:"center" }}><Inbox size={24} style={{ color:C.textFaint }} strokeWidth={1.5} /></div></div>
          ...
          return (
            <div key={a.id} id={`m-ledger-entry-${a.id}`}>
                ...
                <div style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                        <span title={t(SOURCE_CONFIG[a.source]?.labelKey ?? a.source)} style={{ color: SOURCE_CONFIG[a.source]?.color ?? C.textFaint, display: "flex", alignItems: "center", flexShrink: 0 }}>
                          {SOURCE_CONFIG[a.source]?.icon}
                        </span>
                        <span style={{ fontSize: 15, fontWeight: 700, color: C.text, lineHeight: 1.2 }}>{a.name}</span>
                        <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 99, fontWeight: 700, background: (SOURCE_CONFIG[a.source]?.color ?? C.textFaint) + "18", color: SOURCE_CONFIG[a.source]?.color ?? C.textFaint }}>
                          {t(SOURCE_CONFIG[a.source]?.labelKey ?? a.source)}
                        </span>
                        {isLiveRow && <span style={{ fontSize: 10, background: C.redLight, color: C.red, padding: "2px 7px", borderRadius: 99, fontWeight: 800, animation: "gbf-pulse 1s infinite" }}>LIVE</span>}
                      </div>
                      {a.services && a.services.length > 1
                        ? <div style={{ marginTop: 3, display: "flex", flexDirection: "column", gap: 2 }}>
                            {a.services.map((s: ServiceLine, si: number) => (
                              <div key={si} style={{ fontSize: 12, color: C.textMuted }}>
                                {s.service}{s.barber && s.barber !== "TBD" ? <span style={{ color: C.textFaint }}> · {s.barber}</span> : null}
                              </div>
                            ))}
                          </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
                      <div style={{ fontSize: 20, fontWeight: 700, color: a.price > 0 ? C.accent : C.textFaint, lineHeight: 1.1 }}>{a.price > 0 ? `€${a.price}` : "—"}</div>
                      {(a.isAiCall || a.source === "website") && (
                        <span style={{ fontSize: 9, fontWeight: 800, color: "#6C63FF", background: "#6C63FF18", border: "1px solid #6C63FF44", borderRadius: 4, padding: "2px 6px", letterSpacing: "0.04em" }}>AI</span>
                      )}
                      {isAI && <span style={{ fontSize: 11, color: C.textFaint, opacity: isOpen ? 1 : 0.5, transition: "transform .2s", transform: isOpen ? "rotate(180deg)" : "none", display: "inline-block" }}>▾</span>}
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10, gap: 8 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                      <Badge status={a.status} label={tStatus(a.status)} C={C} />
                      {cancelled[a.id] && <span title={`${cancelled[a.id].reason}${cancelled[a.id].note ? ` — ${cancelled[a.id].note}` : ""}`} style={{ fontSize: 11, color: C.textFaint, cursor: "help" }}>ℹ</span>}
                      {a.status !== "cancelled" && a.call_status !== "error" && (
                        <button
                          onClick={e => { e.stopPropagation(); openCancelModal({ ids: [a.id], mode: "single", label: `${a.name} · ${a.service} · ${a.date} ${a.time}` }); }}
                          className="gbf-btn"
                          style={{ fontSize: 11, fontWeight: 700, color: C.red, background: C.redLight, border: `1px solid ${C.red}44`, borderRadius: 8, padding: "8px 14px", cursor: "pointer", fontFamily: "inherit", minHeight: 44, display: "flex", alignItems: "center", gap: 4 }}
                        ><X size={11} /> Cancel</button>
                      )}
                    </div>
                  </div>
                  {isAI && !isOpen && (
                    <div style={{ marginTop: 8, paddingTop: 8, borderTop: `1px solid ${C.borderFaint}`, display: "flex", alignItems: "center", gap: 4 }}>
              ...
              {isOpen && isAI && (
                <div id={`m-ledger-detail-${a.id}`} className="gbf-card-in" style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, borderTop: "none", borderRadius: "0 0 16px 16px", padding: "14px 16px" }}>
                  {loadingCallId === a.conversation_id ? (
...
function RecentCallsSection({ conversations, liveCall, C, density }: {
  ...
  if (recent.length === 0) return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: pad.card }}>
      <div style={{ fontFamily: "var(--gbf-font-display)", fontSize: 17, fontWeight: 600, color: C.text, marginBottom: 6 }}>Recent AI Calls</div>
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: pad.card }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, gap: 8, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontFamily: "var(--gbf-font-display)", fontSize: 17, fontWeight: 600, color: C.text }}>Recent AI Calls</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {recent.map(conv => {
          ...
          return (
            <div key={conv.conversation_id} className="gbf-conv-item" style={{ border: `1px solid ${isLive ? C.amber + "88" : C.borderFaint}`, borderRadius: 12, overflow: "hidden", background: isLive ? C.amberLight : C.surfaceAlt }}>
              {/* Row header */}
              <button
              ...
              >
                ...
                {/* Time */}
                <span className="gbf-recent-time" style={{ fontSize: 12, color: C.textMuted, minWidth: 70, flexShrink: 0 }}>{timeAgo(conv.start_time_unix_secs)}</span>
                {/* Status badge */}
                ...
                {conv.message_count > 0 && (
                  <span className="gbf-recent-msgs" style={{ fontSize: 11, color: C.textFaint, flexShrink: 0 }}>{conv.message_count} msgs</span>
                )}
                {/* Chevron */}
                    ...
                    <div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 10, maxHeight: 320, overflowY: "auto" }}>
                        {(detail.transcript ?? []).map((msg) => {
                          ...
                          return (
                            <div key={msg.time_in_call_secs} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
...
// ── AnalyticsTab ──────────────────────────────────────────────────────────────
function AnalyticsTab({ C, density, conversations, liveCall, businesses, currentBiz, aiBookings }: {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: pad.gap }}>
      ...
      {isAllShopsAnalytics && businesses && (
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 16px", borderRadius:12, background:C.accentLight, border:`1px solid ${C.accent}33` }}>
          ...
          <span style={{ fontSize:12, color:C.textMuted }}>— across {businesses.length} barbershops. Full per-shop breakdown coming soon.</span>
        </div>
      )}
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: pad.card }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, gap: 8 }}>
          <div>
            <div style={{ fontFamily: "var(--gbf-font-display)", fontSize: 17, fontWeight: 600, color: C.text }}>{t("aRevenueTrend")}</div>
      ...
      <div className="gbf-hub-grid">
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: pad.card }}>
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontFamily: "var(--gbf-font-display)", fontSize: 17, fontWeight: 600, color: C.text }}>{t("aAICalls")}</div>
            ...
            {CALL_TREND.map(day => {
              ...
              return (
                <div key={day.label} className="gbf-bar-row" style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span className="gbf-bar-label" style={{ fontSize: 12, color: C.textMuted, width: 30, flexShrink: 0 }}>{day.label}</span>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: pad.card }}>
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontFamily: "var(--gbf-font-display)", fontSize: 17, fontWeight: 600, color: C.text }}>{t("aSvcBreakdown")}</div>
              ...
              <div key={svc.name} className="gbf-bar-row">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 5 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <span className="gbf-bar-label" style={{ fontSize: 13, color: C.text, fontWeight: i < 3 ? 600 : 400 }}>{svc.name}</span>
                  </div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: pad.card }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, gap: 8, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontFamily: "var(--gbf-font-display)", fontSize: 17, fontWeight: 600, color: C.text }}>{t("aBarberPerf")}</div>
        ...
        <div className="gbf-stat-grid gbf-stagger">
          {BARBER_STATS.map((barber, rank) => (
            <div key={barber.name} className="gbf-lift" style={{ background: C.surfaceAlt, borderRadius: 14, padding: "18px 16px", border: `1.5px solid ${rank === 0 ? barber.color + "55" : C.borderFaint}`, position: "relative", overflow: "hidden" }}>
              {/* Rank badge */}
              {rank < 3 && (
                <div style={{ position: "absolute", top: 10, right: 12, width: 22, height: 22, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 800, background: [C.accent, C.textMuted, C.textFaint][rank] + "33", color: [C.accent, C.textMuted, C.textFaint][rank] }}>
                  #{rank + 1}
                </div>
              )}
              {/* Avatar + name */}
              ...
              {/* Top service */}
              <span style={{ fontSize: 11, background: C.accentLight, color: C.accent, padding: "4px 10px", borderRadius: 99, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 4 }}><Sparkles size={10} /> {barber.topService}</span>
              {/* Utilization */}
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: pad.card }}>
        <div style={{ marginBottom: 18 }}>
          <div style={{ fontFamily: "var(--gbf-font-display)", fontSize: 17, fontWeight: 600, color: C.text }}>{t("aPeakHours")}</div>
...
export default function DashboardPage() {
  ...
  const widgetRef = useRef<HTMLDivElement>(null); // kept for gbf-bnav-widget class
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") { setBizOpen(false); setLangOpen(false); }
    };
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      if (!prof?.approved) { router.replace("/dashboard/login?pending=1"); return; }
  ...
  const handleDeleteBiz = useCallback(async (id: string) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const sb = supabase as any;
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const [data, bookingsData, manualData] = await Promise.all([
        ...
        fetch(`/api/elevenlabs/bookings${currentBiz ? `?business_id=${currentBiz.id}` : ""}`, { headers: authHeaders }).then(r => r.json()).catch(() => ({ bookings: [] })),
        currentBiz
          ? fetch(`/api/appointments?business_id=${currentBiz.id}`, { headers: authHeaders }).then(r => r.ok ? r.json() : { bookings: [] }).catch(() => ({ bookings: [] }))
  ...
  useEffect(() => {
    const iv = setInterval(() => { if (!document.hidden) fetchLiveStatus(); }, 10_000);
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => { if (!document.hidden) fetchDashboardData(); }, 30_000);
    return () => clearInterval(iv);
    ...
    try {
      const conv = await Conversation.startSession({
        ...
        onDisconnect: () => {
          ...
          setTimeout(() => { setLiveTranscript([]); setLiveCall(false); }, 6000);
        },
  ...
  const tabs = [
    { id: "hub"       as const, label: t("hubFull"),        short: t("hubShort"),        icon: <LayoutDashboard size={18} strokeWidth={1.75} /> },
    { id: "ledger"    as const, label: t("ledgerFull"),     short: t("ledgerShort"),     icon: <ClipboardList   size={18} strokeWidth={1.75} /> },
    { id: "analytics" as const, label: t("analyticsFull"),  short: t("analyticsShort"),  icon: <BarChart3       size={18} strokeWidth={1.75} /> },
  ];
  ...
  return (
    ...
    <>
      ...
      {/* ── ElevenLabs demo call card ── */}
      <div className={`gbf-call-fab${navBottom ? " gbf-bnav-widget" : ""}`} style={{ position: "fixed", bottom: 24, right: 16, zIndex: 500, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>

        {/* Error toast */}
        {callError && (
          <div onClick={() => setCallError(null)} style={{ maxWidth: "min(280px, calc(100vw - 32px))", background: "#2a1010", border: `1px solid ${C.red}55`, borderRadius: 12, padding: "8px 14px", fontSize: 12, fontWeight: 500, color: C.red, boxShadow: "0 4px 16px rgba(0,0,0,.25)", cursor: "pointer", animation: "gbf-staggerIn .2s ease", lineHeight: 1.4, display: "flex", alignItems: "flex-start", gap: 6 }}>
        ...
        {callStatus === "active" && (
          <div className="gbf-call-card-active">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "11px 14px" }}>
              <div style={{ width: 38, height: 38, borderRadius: "50%", flexShrink: 0, background: "linear-gradient(135deg,#6C5CE7,#E040FB,#06B6D4,#6C5CE7)", backgroundSize: "300% 300%", animation: "gbf-gradFlow 1.8s ease infinite, gbf-pulse 1.2s ease infinite", boxShadow: "0 0 18px rgba(108,92,231,.55)" }} />
              <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {/* End call button */}
            <button onClick={endCall} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, padding: "9px 14px", background: "rgba(239,68,68,.15)", border: "none", borderTop: "1px solid rgba(239,68,68,.2)", color: "#ef4444", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
        ...
        {callStatus === "connecting" && (
          <div style={{ background: "#0e0e1c", border: "1px solid rgba(108,92,231,.32)", borderRadius: 12, padding: "8px 14px", fontSize: 12, fontWeight: 600, color: "#c4b5fd", boxShadow: "0 4px 16px rgba(0,0,0,.25)", animation: "gbf-staggerIn .2s ease", display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 10, height: 10, borderRadius: "50%", background: "linear-gradient(135deg,#6C5CE7,#E040FB)", animation: "gbf-pulse .8s infinite", flexShrink: 0 }} />
            Connecting…
          </div>
        )}
        ...
        {callStatus === "idle" && (
          <button onClick={startCall} className="gbf-call-card">
            ...
            {/* Explanation strip */}
            <div style={{ borderTop: "1px solid rgba(255,255,255,.07)", padding: "7px 14px 9px", fontSize: 10, color: "rgba(255,255,255,.38)", lineHeight: 1.5 }}>
              💬 Live transcription &amp; appointment ledger update in real-time and stay in the dashboard — this call button is temporary (demo trial only)
            </div>
          </button>
        )}
      </div>
      ...
      {settingsOpen && (
        <SettingsPanel
          ...
          onSwitchBiz={(biz) => { setCurrentBiz(biz); setSettingsOpen(false); }}
      ...
      <div dir={isRTL ? "rtl" : "ltr"} className={navBottom ? "gbf-has-bnav" : ""} style={{ minHeight: "100vh", background: C.bg, color: C.text, transition: "background .3s, color .3s" }}>

        {/* Loading progress bar — fixed at very top */}
        {loading && <div className="gbf-progress-top" style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100 }} />}

        {/* Header */}
        <header className="gbf-header" style={{ background: `${C.surface}E8`, backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: `1px solid ${C.border}`, position: "sticky", top: 0, zIndex: 30, transition: "background .3s, border-color .3s", boxShadow: "0 1px 0 rgba(0,0,0,0.04), 0 2px 8px rgba(0,0,0,0.04)" }}>
          <div className="gbf-header-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10, minWidth:0, flexShrink:1 }}>
              <div style={{ width:36, height:36, borderRadius:10, background:C.accentLight, border:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              ...
              {businesses.length >= 1 ? (
                <div style={{ position:"relative" }}>
                  <button
                    ...
                    className="gbf-btn"
                    style={{ display:"flex", alignItems:"center", gap:8, background: bizOpen ? C.accentLight : C.surfaceAlt, border:`1px solid ${bizOpen ? C.accent : C.border}`, borderRadius:10, cursor:"pointer", padding:"6px 10px 6px 10px", fontFamily:"inherit", textAlign:"left", transition:"background .15s, border-color .15s" }}
                  >
                    <Store size={14} style={{ color: C.accent, flexShrink:0 }} />
                    <div>
                      <div style={{ fontFamily:"var(--gbf-font-display)", fontSize:14, fontWeight:700, color:C.text, lineHeight:1.2, letterSpacing:"-0.01em", whiteSpace:"nowrap" }}>
                        {currentBiz?.name ?? "All shops"}
                      </div>
                      <div style={{ fontSize:10, color:C.textMuted, letterSpacing:"0.04em", fontWeight:500 }}>{businesses.length} barbershop{businesses.length !== 1 ? "s" : ""}</div>
                    </div>
                    <ChevronDown size={13} style={{ color:C.textFaint, transition:"transform .15s", transform: bizOpen ? "rotate(180deg)" : "none", flexShrink:0 }} />
                  </button>
                  {bizOpen && (
                    <>
                      {createPortal(<div onClick={() => setBizOpen(false)} style={{ position:"fixed", inset:0, zIndex:25 }} />, document.body)}
                      <div style={{ position:"absolute", top:"calc(100% + 8px)", left:0, right:"auto", maxWidth:"calc(100vw - 32px)", zIndex:201, background:C.surface, border:`1px solid ${C.border}`, borderRadius:14, boxShadow:"0 8px 32px rgba(0,0,0,.18)", padding:8, minWidth:220, animation:"gbf-fadeIn .15s ease" }}>
                        <div style={{ fontSize:10, fontWeight:700, color:C.textFaint, textTransform:"uppercase", letterSpacing:"0.08em", padding:"4px 12px 8px" }}>Your Shops</div>
                        {/* All shops option */}
                        <button onClick={() => { setCurrentBiz(null); setBizOpen(false); }} className="gbf-btn gbf-menu-item" style={{ display:"flex", alignItems:"center", gap:10, width:"100%", padding:"9px 12px", borderRadius:10, border:"none", background: currentBiz===null ? C.accentLight : "transparent", cursor:"pointer", fontFamily:"inherit", textAlign:"left" }}>
                          <LayoutDashboard size={14} style={{ color: currentBiz===null ? C.accent : C.textMuted, flexShrink:0 }} />
                          <span style={{ fontSize:13, fontWeight: currentBiz===null ? 700 : 500, color: currentBiz===null ? C.accent : C.text }}>All shops</span>
                          {currentBiz===null && <Check size={13} style={{ color:C.accent, marginLeft:"auto", flexShrink:0 }} strokeWidth={2.5} />}
                        </button>
                        <div style={{ height:1, background:C.borderFaint, margin:"4px 0" }} />
                        {businesses.map(biz => (
                          <button key={biz.id} onClick={() => { setCurrentBiz(biz); setBizOpen(false); }} className="gbf-btn gbf-menu-item" style={{ display:"flex", alignItems:"center", gap:10, width:"100%", padding:"9px 12px", borderRadius:10, border:"none", background: currentBiz?.id===biz.id ? C.accentLight : "transparent", cursor:"pointer", fontFamily:"inherit", textAlign:"left" }}>
                            <Store size={14} style={{ color: currentBiz?.id===biz.id ? C.accent : C.textMuted, flexShrink:0 }} />
                            <span style={{ fontSize:13, fontWeight: currentBiz?.id===biz.id ? 700 : 500, color: currentBiz?.id===biz.id ? C.accent : C.text }}>{biz.name}</span>
                            {currentBiz?.id===biz.id && <Check size={13} style={{ color:C.accent, marginLeft:"auto", flexShrink:0 }} strokeWidth={2.5} />}
                          </button>
                        ))}
                        <div style={{ height:1, background:C.borderFaint, margin:"4px 0" }} />
                        <button onClick={() => { setBizOpen(false); setAddShopOpen(true); }} className="gbf-btn gbf-menu-item" style={{ display:"flex", alignItems:"center", gap:10, width:"100%", padding:"9px 12px", borderRadius:10, border:"none", background:"transparent", cursor:"pointer", fontFamily:"inherit", textAlign:"left" }}>
                ...
                <button onClick={() => window.scrollTo({ top:0, behavior:"smooth" })} style={{ display:"flex", flexDirection:"column", background:"none", border:"none", cursor:"pointer", padding:0, fontFamily:"inherit", textAlign:"left", minWidth:0 }}>
                  <div style={{ fontFamily:"var(--gbf-font-display)", fontSize:16, fontWeight:700, color:C.text, lineHeight:1.2, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", letterSpacing:"-0.01em" }}>
                    {currentBiz?.name ?? profile.businessName ?? "Greek Barber Festival"}
                  </div>
                  <div style={{ fontSize:10, color:C.textMuted, letterSpacing:"0.08em", textTransform:"uppercase", fontWeight:500 }}>{t("aiDashboard")}</div>
                </button>
              )}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
              ...
              <div style={{ position: "relative", flexShrink: 0 }}>
                ...
                {langOpen && (
                  <>
                    {createPortal(<div onClick={() => setLangOpen(false)} style={{ position: "fixed", inset: 0, zIndex: 25 }} />, document.body)}
                    <div style={{
                      ...
                      {(Object.entries(LANG_META) as [LanguageKey, typeof LANG_META[LanguageKey]][]).map(([key, meta]) => {
                        const active = langSettings.lang === key;
                        return (
                          <button key={key} onClick={() => { updateLangSettings({ lang: key }); setLangOpen(false); }} className="gbf-btn gbf-menu-item" style={{
                            ...
                            {active && <Check size={14} style={{ color: C.accent, flexShrink: 0 }} strokeWidth={2.5} />}
                          </button>
                        );
                      })}
                    </div>
              ...
              {/* Walk-in quick entry */}
              <button onClick={() => setWalkinOpen(true)} title="Register walk-in" className="gbf-btn gbf-action-btn" style={{ display:"flex", alignItems:"center", gap:6, height:40, padding:"0 14px", borderRadius:10, border:`1px solid ${C.accent}44`, background:C.accentLight, color:C.accent, cursor:"pointer", fontFamily:"inherit", fontWeight:700, fontSize:13, flexShrink:0, transition:"all .15s" }}>
              ...
              {userEmail && (
                <div style={{ display:"flex", alignItems:"center", gap:6, flexShrink:0 }}>
                  <span className="gbf-header-date" style={{ fontSize:12, color:C.textFaint, maxWidth:120, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{userEmail}</span>
                  <button onClick={handleLogout} title="Sign out" className="gbf-btn gbf-icon-btn" style={{ display:"flex", alignItems:"center", justifyContent:"center", width:36, height:36, borderRadius:10, border:`1px solid ${C.border}`, background:C.surfaceAlt, color:C.textMuted, cursor:"pointer", flexShrink:0, transition:"all .15s" }}>
                    <LogOut size={15} strokeWidth={2} />
                  </button>
                </div>
              )}

              <button onClick={() => setSettingsOpen(true)} title="Settings" className="gbf-btn gbf-icon-btn" style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 40, height: 40, borderRadius: 10, border: `1px solid ${C.border}`, background: C.surfaceAlt, color: C.textMuted, cursor: "pointer", flexShrink: 0, transition: "all .15s" }}>
                <GearIcon />
              </button>
            </div>
          </div>
        ...
        {/* Live call banner */}
        {liveCall && (
          <div style={{ background: C.red, color: "#fff", padding: "8px 20px", display: "flex", alignItems: "center", gap: 10, fontSize: 13, fontWeight: 600, flexWrap: "wrap" }}>
            ...
            <button onClick={() => setTab("analytics")} style={{ marginLeft: "auto", fontSize: 11, fontWeight: 700, opacity: 0.9, background: "rgba(255,255,255,.18)", border: "none", color: "#fff", padding: "4px 12px", borderRadius: 99, cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}>View Analytics <ArrowRight size={11} strokeWidth={2.5} style={{ display:"inline", verticalAlign:"middle", marginLeft:2 }} /></button>
          </div>
        )}
        ...
        {!navBottom && (
          <div className="gbf-topnav" style={{ background: C.surface, borderBottom: `1px solid ${C.border}` }}>
            <div className="gbf-tabs-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", gap: 2 }}>
              {tabs.map(tb => (
                <button key={tb.id} onClick={() => setTab(tb.id)} className="gbf-tab-btn gbf-btn" aria-current={tab === tb.id ? "page" : undefined} style={{ fontWeight: tab === tb.id ? 700 : 500, color: tab === tb.id ? C.accent : C.textMuted, background: tab === tb.id ? C.accentLight : "transparent", border: "none", borderBottom: tab === tb.id ? `2px solid ${C.accent}` : "2px solid transparent", borderRadius: "10px 10px 0 0", cursor: "pointer", fontFamily: "inherit", transition: "color .2s, background .2s", display: "flex", alignItems: "center", gap: 7, letterSpacing: "-0.01em" }}>
                  <span className="gbf-tab-icon" style={{ display: "flex", alignItems: "center" }}>{tb.icon}</span>
        ...
        <div className={`gbf-content${navBottom ? " gbf-content-bnav" : ""}`} style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: tab === "hub"       ? "block" : "none" }} className="gbf-tab-content">
            <HubTab agent={agent} C={C} density={settings.density} loading={loading} profile={profile} conversations={conversations} aiBookings={aiBookings} liveCall={liveCall}
              onTabChange={setTab}
              onConvSelect={(id) => { setSelectedConvId(id); setTab("ledger"); }}
              callActive={callActive} callStatus={callStatus} liveTranscript={liveTranscript} agentMode={agentMode} onHumanTakeover={handleHumanTakeover}
              businesses={businesses} currentBiz={currentBiz} />
          </div>
          <div style={{ display: tab === "ledger"    ? "block" : "none" }} className="gbf-tab-content">
            <LedgerTab key={currentBiz?.id ?? "all"} C={C} density={settings.density} aiBookings={[...aiBookings, ...manualBookings]} liveCall={liveCall}
              selectedConvId={selectedConvId} onConvSelected={() => setSelectedConvId(null)}
              businesses={businesses} isAllShops={currentBiz === null && businesses.length > 1}
              currentBiz={currentBiz} onRefreshData={fetchDashboardData} />
          </div>
          <div style={{ display: tab === "analytics" ? "block" : "none" }} className="gbf-tab-content">
            <AnalyticsTab C={C} density={settings.density} conversations={conversations} liveCall={liveCall}
              businesses={businesses} currentBiz={currentBiz} aiBookings={aiBookings} />
          </div>
        </div>
        ...
        {navBottom && (
          <div className="gbf-nav-bottom" style={{ background: `${C.surface}F0`, backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)" }}>
            <div className="gbf-tabs-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex" }}>
              {tabs.map(tb => (
                <button key={tb.id} onClick={() => setTab(tb.id)} className="gbf-tab-btn gbf-btn" aria-current={tab === tb.id ? "page" : undefined} style={{ fontWeight: tab === tb.id ? 700 : 500, color: tab === tb.id ? C.accent : C.textMuted, background: tab === tb.id ? C.accentLight : "transparent", border: "none", borderRadius: 14, cursor: "pointer", fontFamily: "inherit", transition: "color .2s, background .2s", display: "flex", alignItems: "center", flexDirection: "column", flex: 1, gap: 2 }}>
                  <span className="gbf-tab-icon" style={{ margin: 0, display: "flex", alignItems: "center" }}>{tb.icon}</span>
                  <span className="gbf-tab-full" style={{ letterSpacing: "-0.01em" }}>{tb.label}</span>
                  <span className="gbf-tab-short" style={{ fontSize: 10, letterSpacing: "-0.01em" }}>{tb.short}</span>
                </button>
              ))}
            </div>
          </div>

...
const C = {
  bg: "#0E1510", surface: "#161C14", surfaceAlt: "#1E2A1C",
  border: "#2A3828", accent: "#8FBF7F", accentMid: "#6A9A5C",
  accentLight: "#1E2A1C", text: "#DCF0D4", textMuted: "#8AAC80",
  textFaint: "#567050", red: "#E07070", redLight: "#3A1818",
};
...
function LoginInner() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (!session) return;
      const { data: profileRaw } = await supabase.from("profiles").select("approved").eq("id", session.user.id).single();
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    const profile = profileRaw as { approved: boolean } | null;
    if (!profile?.approved) { setView("pending"); setLoading(false); return; }
  ...
  const inp: React.CSSProperties = {
    ...
    background: C.surfaceAlt, color: C.text, fontSize: 15, fontFamily: "inherit",
  ...
  const btn = (disabled = false): React.CSSProperties => ({
    ...
    fontFamily: "inherit", transition: "background .15s, transform .1s", width: "100%",
    ...
    <div style={{
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{
            width: 60, height: 60, borderRadius: 18, background: C.surfaceAlt,
            border: `1px solid ${C.border}`, display: "flex", alignItems: "center",
            justifyContent: "center", margin: "0 auto 14px",
            boxShadow: `0 0 0 8px ${C.accent}12`,
          }}><Scissors size={26} style={{ color: C.accent }} /></div>
          <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 700, color: C.text }}>
            Barber Dashboard
          </div>
          <div style={{ fontSize: 11, color: C.textFaint, marginTop: 4, letterSpacing: "0.1em", textTransform: "uppercase" }}>
        ...
        <div style={{
          ...
          {view === "pending" && (
            <div style={{ textAlign: "center", padding: "8px 0 12px" }}>
              <div style={{ marginBottom: 12, display: "flex", justifyContent: "center" }}><Clock size={40} style={{ color: C.textMuted }} /></div>
              ...
              <form onSubmit={handleLogin} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div>
                  ...
                  <input ref={emailRef} type="email" value={email} onChange={e => setEmail(e.target.value)}
                    placeholder="demo@barbershop.com" required style={inp} autoComplete="email" />
                </div>
                <div>
                  <label style={label}>Password</label>
                  <div style={{ position: "relative" }}>
                    <input type={showPass ? "text" : "password"} value={password} onChange={e => setPass(e.target.value)}
                      placeholder="••••••••" required style={{ ...inp, paddingRight: 44 }} autoComplete="current-password" />
                    <button type="button" onClick={() => setShowPass(p => !p)} style={{
                      position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)",
                      background: "none", border: "none", color: C.textFaint, cursor: "pointer", display: "flex", alignItems: "center",
                    }}>{showPass ? <EyeOff size={16} /> : <Eye size={16} />}</button>
                  </div>
                </div>
                {error && <div style={{ fontSize: 13, color: C.red, background: C.redLight, padding: "10px 14px", borderRadius: 8 }}>{error}</div>}
                <button type="submit" disabled={loading} style={btn(loading)}>
                  {loading ? "Signing in…" : "Sign in"}
                </button>
              </form>
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 16, fontSize: 13 }}>
                <button onClick={() => { setView("forgot"); setError(""); }} style={{ background: "none", border: "none", color: C.textMuted, cursor: "pointer", fontFamily: "inherit" }}>
                  Forgot password?
                </button>
                <button onClick={() => { setView("signup"); setError(""); }} style={{ background: "none", border: "none", color: C.accent, cursor: "pointer", fontFamily: "inherit", fontWeight: 700 }}>
                  Create account →
                </button>
              </div>
            </>
            ...
            <>
              ...
              <form onSubmit={handleSignup} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div>
                  ...
                  <input ref={emailRef} type="text" value={name} onChange={e => setName(e.target.value)}
                    placeholder="Your name" required style={inp} />
                </div>
                <div>
                  <label style={label}>Email</label>
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                    placeholder="you@barbershop.com" required style={inp} />
                </div>
                <div>
                  <label style={label}>Password</label>
                  <input type={showPass ? "text" : "password"} value={password} onChange={e => setPass(e.target.value)}
                    placeholder="Min. 8 characters" minLength={8} required style={inp} />
                </div>
                {error && <div style={{ fontSize: 13, color: C.red, background: C.redLight, padding: "10px 14px", borderRadius: 8 }}>{error}</div>}
                <button type="submit" disabled={loading} style={btn(loading)}>
              ...
              <button onClick={() => { setView("login"); setError(""); }} style={{
                ...
                fontFamily: "inherit", fontSize: 13, marginTop: 14, padding: 0,
          ...
          {view === "forgot" && (
            <>
              ...
              <form onSubmit={handleForgot} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                {error   && <div style={{ fontSize: 13, color: C.red,   background: C.redLight,   padding: "10px 14px", borderRadius: 8 }}>{error}</div>}
                {success && <div style={{ fontSize: 13, color: C.accent, background: C.accentLight, padding: "10px 14px", borderRadius: 8, border: `1px solid ${C.accent}33` }}>{success}</div>}
                <button type="submit" disabled={loading} style={btn(loading)}>
                  {loading ? "Sending…" : "Send reset link"}
                </button>
              </form>
              <button onClick={() => { setView("login"); setError(""); setSuccess(""); }} style={{
                background: "none", border: "none", color: C.textMuted, cursor: "pointer",
                fontFamily: "inherit", fontSize: 13, marginTop: 14, padding: 0,
              }}>← Back to sign in</button>
            </>
        ...
        {view === "login" && (
          <button
            type="button"
            onClick={() => { setEmail("demo@barbershop.com"); setPass("demo1234"); }}
            style={{
              marginTop: 14, padding: "14px 16px", borderRadius: 14, width: "100%",
              background: `${C.accent}0D`, border: `1.5px solid ${C.accent}30`,
          ...
          >
            ...
            <div style={{
              ...
              background: `${C.accent}18`, padding: "5px 10px", borderRadius: 8,
            }}>

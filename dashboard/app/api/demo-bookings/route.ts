...
const CLIENTS_F = ["Alexandros","Dimitrios","Nikolaos","Georgios","Konstantinos","Ioannis","Vasileios","Christos","Athanasios","Michail","Theodoros","Spyridon","Evangelos","Andreas","Petros","Stelios","Nikos","Kostas","Takis","Manolis","Makis","Sakis","Giannis","Thanasis","Giorgos","Leonidas","Thanos","Babis","Lefteris"];
const CLIENTS_L = ["Papadopoulos","Papadimitriou","Nikolaou","Georgiou","Konstantinidis","Ioannou","Alexiou","Vasiliou","Christodoulou","Petrou","Stavropoulos","Papageorgiou","Anastasiou","Karamanlis","Dimos","Lambrakis","Zografos","Mpournazos"];
...
const NOT_A_NAME  = new Set(["the","a","an","and","or","of","at","in","on","by","for","to","de","la","el","los","das","die","der","και"]);
...
function fallbackBarbers(shopName: string): string[] {
  const seed = shopName.trim().split(/\s+/).find(
    w => w.length >= 3 && /^[A-Z]/.test(w) && !NOT_A_NAME.has(w.toLowerCase()),
  ) ?? null;
...
export async function POST(req: Request) {
  ...
  let sourceIdx = 0;
  function nextSource(): string { return BASE_SOURCES[sourceIdx++ % BASE_SOURCES.length]; }
  ...
  function pickStatus(source: string): { callStatus: string; apptStatus: string; callOk: string | null } {
    ...
    if (source === "agent") {
      ...
      else if (r < 0.60) return { callStatus: "done",      apptStatus: "confirmed",  callOk: "true"  };
      else if (r < 0.70) return { callStatus: "done",      apptStatus: "cancelled",  callOk: "true"  };
      else if (r < 0.80) return { callStatus: "missed",    apptStatus: "no_show",    callOk: "false" };
      else if (r < 0.90) return { callStatus: "cancelled", apptStatus: "cancelled",  callOk: "false" };
      else               return { callStatus: "failed",    apptStatus: "failed",     callOk: "false" };
    }
    if (source === "walk-in") {
      // Walk-ins almost always show up
      if      (r < 0.70) return { callStatus: "done", apptStatus: "completed",  callOk: null };
    ...
    if (source === "website") {
      ...
      if      (r < 0.50) return { callStatus: "done", apptStatus: "confirmed",  callOk: null };
  ...
  for (let i = 0; i < count; i++) {
    ...
    const svcCount = isFailed ? 1
      : source === "website"   ? (svcRnd < 0.80 ? 1 : 2)
      : source === "walk-in"   ? (svcRnd < 0.40 ? 1 : svcRnd < 0.75 ? 2 : 3)
      : (svcRnd < 0.50 ? 1 : svcRnd < 0.80 ? 2 : 3);
  ...
  if (errors.length) {
    return NextResponse.json({ warning: errors.join("; "), inserted: { calls: callRows.length, appointments: apptRows.length } });
  }
